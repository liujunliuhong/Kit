...
#ifdef DEBUG
    #define YHDebugLog(format, ...)  printf("[YHDebugLog] [YHChineseSort] [%s] [%d] %s\n" ,[[[NSString stringWithUTF8String:__FILE__] lastPathComponent] UTF8String], __LINE__, [[NSString stringWithFormat:format, ##__VA_ARGS__] UTF8String])
#else
...
- (void)sortWithModels:(NSArray *)models key:(NSString *)key modelClass:(Class)modelClass completion:(void (^)(NSArray<NSArray *> * _Nonnull, NSArray<NSString *> * _Nonnull))completion{
    ...
    dispatch_async(dispatch_get_global_queue(0, 0), ^{
        ...
        YHDebugLog(@"Get first letter total cost %fs",end - start);
        ...
        YHDebugLog(@"Sort ascending total cost %fs",end1 - end);
        ...
        YHDebugLog(@"Sort total cost %fs",end2 - end1);
        YHDebugLog(@"Total cost %fs",end2 - start);
